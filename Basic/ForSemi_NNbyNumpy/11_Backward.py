#A Simple Task:3. BackwardFunction - demand func + loss func
import random
import matplotlib.pyplot as plt
import copy
import numpy as np
from tools import normalize, activation_softmax, activation_ReLu
...
def get_final_layer_preAct_demands(predicted_values, target_vector):
  ...
  for i in range(len(target_vector)):
    if np.dot(target[i], predicted_values[i]) > 0.5: #the cross-entropy loss function is used to quantify the difference
      target[i] = np.array([0, 0])
    else:
      target[i] = (target[i] - 0.5) * 2 #If left < right, need to adjust left [-1, 1] otherwise [1, -1]
  return target
...
def precise_loss_function(predicted, real):
  ...
  #loss = -np.sum(real_matrix * np.log(predicted)) #Hadamard product
  loss = np.sum(predicted * real_matrix, axis = 1) #
  return 1 - loss #The bigger number, the larger error, the loss calculation need to be reversed.
